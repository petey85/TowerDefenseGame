import pygame as pg # use 'pg' when calling pygame
import json
from enemy import Enemy
from world import World
from turret import Turret
from button import Button
import constants as c # use 'c' when calling constants
...
#create game window
screen = pg.display.set_mode((c.SCREEN_WIDTH + c.SIDE_PANEL, c.SCREEN_HEIGHT)) # height and width of game window
pg.display.set_caption("Tower Defense") # game title window name

#game variables
game_over = False
game_outcome = 0 # -1 is a loss & 1 is a win
level_started = False
last_enemy_spawn = pg.time.get_ticks() #timer to see when to produce new enemy spawn
placing_turrets = False #acts as a trigger to determine if we are placing turrets or not
...
#turret spritesheets
turret_spritesheets = []
for x in range(1, c.TURRET_LEVELS + 1): # whenever you use range in a for loop, it goes up to but not including the 2nd variable, hence the + 1 at the end
...
#load sounds
shot_fx = pg.mixer.Sound('assets/audio/laser2.ogg')
shot_fx.set_volume(0.5) #50 percent of original volume
...
def draw_text(text, font, text_col, x, y):
    img = font.render(text, True, text_col) #converts text into an image
    screen.blit(img, (x, y))
...
def display_data():
    ...
    #display data
    draw_text("LEVEL: " + str(world.level), text_font, "grey100", c.SCREEN_WIDTH + 10, 10) #convert the level integer into a string
    screen.blit(heart_image, (c.SCREEN_WIDTH + 10, 35))
    draw_text(str(world.health), text_font, "grey100", c.SCREEN_WIDTH + 50, 40) #convert the health integer into a string
    screen.blit(coin_image, (c.SCREEN_WIDTH + 10, 65))
    draw_text(str(world.money), text_font, "grey100", c.SCREEN_WIDTH + 50, 70) #convert the money integer into a string
...
def create_turret(mouse_pos):
    ...
    if world.tile_map[mouse_tile_num] == 7:
        ...
        if space_is_free == True:
            ...
            world.money -= c.BUY_COST #reduces total money by the BUY_COST constant
...
#create buttons
turret_button = Button(c.SCREEN_WIDTH + 30, 120, buy_turret_image, True) #create new turret button 30 pixels to right into side panel
...
while run:
    ...
    if game_over == False:    
        ...
        if level_started == False:
            ...
            if pg.time.get_ticks() - last_enemy_spawn > c.SPAWN_COOLDOWN:
                if world.spawned_enemies < len(world.enemy_list):
                    ...
                    world.spawned_enemies += 1 # this ensures us that we will go thru the whole list
        ...
        if world.check_level_complete() == True:
            ...
            level_started = False
            last_enemy_spawn = pg.time.get_ticks()
            world.reset_level() # sets everything back and clears out the level
            world.process_enemies()
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.QUIT:
            run = False
        #mouse click
        if event.type == pg.MOUSEBUTTONDOWN and event.button == 1: # 1 is left click button
            mouse_pos = pg.mouse.get_pos() # get x/y coords of mouse position
